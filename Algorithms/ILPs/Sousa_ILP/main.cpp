...
int bfs(vector<set<int>> &graph, set<int> source){
	
	vector<int> dist(int(graph.size()), 1<<20);
	queue<int> q;
	int maxDist = 0;
	...
	*/
	...
	try {
		...
		if( upperBound == -1 ){
			...
			asnwerHeuristic = min(upperBound + 1, qtVertex);
		}
		...
		if( lowerBound ) {
			lowerBound = ceil(log2(qtVertex/(1.0*qtUpdated)));
			if(LowerBound) {
				lowerBound = max(bfs(graph, updateds), lowerBound);
			}
		}
		...
		// cplex.setParam(IloCplex::TreLim, 1024*1.5);
		//	-	z
		...
		outputFile << "Status: " << cplex.getCplexStatus() << endl;
