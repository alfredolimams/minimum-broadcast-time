...
#ifndef Decoder_FRFSI0_H
#define Decoder_FRFSI0_H

#include "Decoder_MBT.hpp"

using namespace std;

class Decoder_FRFSI0 : public Decoder_MBT {
public:
	Decoder_FRFSI0(string name);
	Decoder_FRFSI0(double pe, double pm, double rhoe);
	~Decoder_FRFSI0();
	double decode(const vector< double >& chromosome) const;
	Forest build_forest(const std::vector< double >& chromosome) const;
