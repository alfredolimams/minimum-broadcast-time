...
public:
	...
	/**
	 * Set chromosome the current population
	 */
	bool setChromosome(unsigned k, unsigned i, std::vector<double> &chromosome, double d);
...
template< class Decoder, class RNG >
bool BRKGA< Decoder, RNG >::setChromosome(unsigned k, unsigned i, std::vector<double> &chromosome, double d){
	if ( k >= this->getP() ) return false;
	return current[k]->setChromosome(i,chromosome,d);
}
