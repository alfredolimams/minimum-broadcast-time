...
public:
	...
	bool setChromosome(unsigned i, std::vector< double >& chromosome, double fitness);	// Set chromosome and fitness
...
bool Population::setChromosome(unsigned i, std::vector< double >& chromosome, double f){
	if( i < 0 or i >= this->getP() ) return false;
	if( chromosome.size() != this->getN() ) return false;

	for ( unsigned j = 0 ; j < chromosome.size() ; ++j ){
		population[ fitness[i].second ][j] = chromosome[j];
	}
	this->setFitness(i,f);
	// this->sortFitness();
	return true;
